...
const contactData = (props) => {
  const [state, setConState] = useState({
    orderForm: {
      name: {
        elementType: "input",
        elementConfig: {
  ...
  const orderHandler = useCallback(
    (event) => {
      ...
      for (const key in state.orderForm) {
        orderdata[key] = state.orderForm[key].value;
      }
      const data = {
      ...
      props.postOrderData(props.token, data);
    },
    [state, props]
  );
  ...
  const changedInputHandler = useCallback(
    (event, elementIdentifer) => {
      const updatedFormElement = updateObject(
        state.orderForm[elementIdentifer],
        {
          value: event.target.value,
          valid: checkValidity(
            event.target.value,
            state.orderForm[elementIdentifer].validation
          ),
          touched: true,
        }
      );
      ...
      setConState({ orderForm: updatedOrderForm, formIsValid: isValid });
    },
    [state, props]
  );
  ...
  let form = (
    <form onSubmit={orderHandler}>
      {Object.keys(state.orderForm).map((element, index) => (
        <Input
          key={index}
          elementType={state.orderForm[element].elementType}
          elementConfig={state.orderForm[element].elementConfig}
          changed={(event) => changedInputHandler(event, element)}
          shouldvalidate={state.orderForm[element].validation}
          touched={state.orderForm[element].touched}
          invalid={!state.orderForm[element].valid}
        />
      ))}
      <Button disabled={!state.formIsValid} btnTybe="Success">
        ORDER
      </Button>
    </form>
  );
