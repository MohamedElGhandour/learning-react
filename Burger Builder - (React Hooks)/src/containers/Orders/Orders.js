    ...
    if (props.orders) {
      if (
        !(
          ...
          props.orders.constructor === Object
        )
      ) {
        orders = Object.keys(props.orders).map((order, index) => (
          <Order
            delete={(id) => props.onDeleteOrder(props.token, id)}
            key={index}
            id={order}
            customer={props.orders[order].orderdata}
            ingredient={props.orders[order].ingredient}
